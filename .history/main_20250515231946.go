...
func cariSaham() {
	...
	found := false
	// Menggunakan pendekatan iterasi manual tanpa len
	i := 0
	for {
		// Cek apakah indeks masih valid dalam slice
		if i >= 0 && i < 5 { // Kita tahu listSaham memiliki 5 elemen
			s := listSaham[i]
			if s.Kode == kodeSaham {
				fmt.Printf("Kode: %s, Nama: %s, Harga: %.2f, Kategori: %s\n", s.Kode, s.Nama, s.Harga, s.Kategori)
				found = true
			...
			break // Keluar dari loop jika sudah melebihi jumlah elemen
		}
	}
