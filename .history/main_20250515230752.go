...
func selectionSortByPrice(saham []Saham) []Saham {
	// Membuat salinan slice saham untuk tidak mengubah data asli
...
func daftarSaham() {
	...
	// Di sini, 'range listSaham' mengiterasi seluruh slice listSaham
	// Setiap iterasi, 's' berisi nilai elemen (Saham) saat ini
	// '_' adalah indeks elemen yang tidak digunakan dalam kasus ini
	for _, s := range listSaham {
		fmt.Printf("Kode: %s, Nama: %s, Harga: %.2f, Kategori: %s\n", s.Kode, s.Nama, s.Harga, s.Kategori)
	}
	
	// Contoh alternatif menggunakan for loop tradisional:
	/*
...
func cariSaham() {
	...
	found := false
	// Sama seperti di atas, '_' adalah indeks, 's' adalah nilai elemen saat ini
	for _, s := range listSaham {
		if s.Kode == kodeSaham {
			fmt.Printf("Kode: %s, Nama: %s, Harga: %.2f, Kategori: %s\n", s.Kode, s.Nama, s.Harga, s.Kategori)
			found = true
