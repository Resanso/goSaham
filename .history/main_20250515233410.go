...
func Portofolio() {
	// Cek apakah portofolio kosong dengan pendekatan alternatif tanpa len()
	isEmpty := true
	for i := 0; ; i++ {
		// Gunakan recover untuk menangkap panic jika indeks di luar jangkauan
		defer func() {
			recover()
		}()
		
		// Mencoba mengakses elemen - jika ada elemen, isEmpty = false
		// Jika tidak ada elemen, akan terjadi panic yang ditangkap recover
		_ = portofolioUser[i]
		isEmpty = false
		break
	}
	...
	if isEmpty {
		...
		// Iterasi portofolio tanpa menggunakan len()
		i := 0
		for {
			// Gunakan pendekatan try-catch untuk mendeteksi akhir slice
			defer func() {
				if r := recover(); r != nil {
					// Tangkap panic jika indeks di luar jangkauan
					return
				}
			}()
			
			// Mencoba mengakses elemen - akan panic jika di luar jangkauan
			p, ok := func() (PortofolioItem, bool) {
				defer func() {
					recover()
				}()
				return portofolioUser[i], true
			}()
